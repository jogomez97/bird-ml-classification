...
class CustomDatasetLoaderAndSplitter:
    ...
    def __split(self):
        ...
        for d, folder in cuts_per_file.items():
            ...
            while not successfull:
                ...
                for x, p in list(prob_dict.items()):
                    ...
                    if p_total < self.train_split:
                        ...
                        prob_dict.pop(x)
                        continue
                    if p_total == self.train_split or (
                        p_total > self.train_split
                        and p_total <= self.train_split + 0.01
                    ):
                        train_dict[d].append(x)
                        prob_dict.pop(x)
                        break
            ...
            if self.test == 0:
                ...
                if self.verbose:
                    print(
                        "Real split: train {}%, validation {}%, test {}%".format(
                            real_train_split, real_val_split, 0
                        )
                    )
            ...
            for x, p in list(prob_dict.items()):
                ...
                if p_total < self.validation:
                    ...
                    prob_dict.pop(x)
                    continue
                if p_total == self.validation or (
                    p_total > self.validation and p_total <= self.validation + 0.01
                ):
                    val_dict[d].append(x)
                    prob_dict.pop(x)
                    break
            ...
            if self.verbose:
                print(
                    "Real split: train {}%, validation {}%, test {}%".format(
                        real_train_split, real_val_split, real_test_split
                    )
                )
    ...
    def __load(self, image_dict, verbose=-1):
            ...
            for image_name in arr:
                ...
                if self.verbose:
                    if verbose > 0 and i > 0 and (i + 1) % verbose == 0:
                        print(
                            "\r[Dataset Loader] processed {}/{} {}\t\t\t\t\t\t\t\t".format(
                                i + 1, len(arr), label
                            ),
                            end="",
                        )
        ...
        # randomly shuffle the output data
